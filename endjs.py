...
def testcookie():
    fw = open('testcookie.txt', 'w', encoding='UTF-8')
    fw.write("true")
    fw.close()
    ...
    def read_info(self):
        ...
        open(f"./token.txt", "a+", encoding="utf-8-sig").close()
        ...
        open(f"./cookies.txt", "a+", encoding="utf-8-sig").close()
        with open(f"./cookie.txt", "r+", encoding="utf-8-sig") as file:
            self.cookie = file.read()
        with open(f"./token.txt", "r+", encoding="utf-8-sig") as file:
        ...
        with open(f"./info.json", "r+", encoding="utf-8-sig") as file:
            info = file.read()
        with open(f"./cookies.txt", "r+", encoding="utf-8-sig") as file:
            self.cookies = file.read()
        if info:
        ...
        if not self.cookie or not self.openid or not self.sid:
            ...
            self.headerd['Cookie'] = self.cookies
            url = f"http://xyc.q2q365.com/qckj/seat/person?sid={self.sid}&openid={self.openid}"
    ...
    def run(self):
        ...
        url = f"http://xyc.q2q365.com/qckj/seat/seat_status?sid={self.sid}&openid={self.openid}&l_id={select}"
        self.pprint("\n尝试进入房间...")
        result = requests.get(url, headers=self.headers).text
    ...
    def cancel(self):
        self.pprint("\n检查座位状态...")
        url = f'http://xyc.q2q365.com/qckj/seat/person?sid={self.sid}&openid={self.openid}&r=' + str(
            int(time.time()))
    ...
    def select_submit(self, url, seat):
        ...
        while True:
            ...
            if "预约成功后需在30分钟内签到" in result:
                ...
                if search:
                    self.pprint(url)
                    requests.get(self.token, headers=self.headerd, timeout=5)
    ...
    def time_check(self):
        target_time = datetime.time(6, 30, 00)
        end_time = datetime.time(20, 50, 00)
        current_time = datetime.datetime.now().time()
        if current_time > end_time:
            self.end(f"\n当前时间为【{self.get_time()}】\n已经过了抢座时间，请明天再来！")

        if current_time < target_time:
            self.pprint(f"未到抢座时间，设定06:30:00开始运行占座程序...\n")
        while True:
            current_time = datetime.datetime.now().time()
